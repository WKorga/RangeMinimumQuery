...
public class Main {
    ...
    private static int[] superBlocksMinima;
    private static int[][] blocksMinima;
    private static int[][] superBlockSparseTable;
    private static int[][][] blockSparseTable;
    private static int[][][] answers;
    private static int blockSize;
    private static int superblockSize;
    ...
    private static void buildBTRLArrays(){
        /**
    ...
    private static void searchTree(Node node, int depth){
        T.add(node.getIndex());
        L.add(depth);
        B.set(T.size()-1);
        R[node.getIndex()]=T.size()-1;
    ...
    private static void buildMinimaTables(){
        /**
        ...
        blocksMinima[superBlocksMinima.length-1] =
                new int[T.size()%superblockSize==0?superblockSize/blockSize:
                        (int)(Math.ceil(T.size()%superblockSize/(double)blockSize))];
    ...
    private static void buildSparseTables(){
        /**
        ...
        for (int i=0; i<superBlockSparseTable.length;i++){
            ...
            for (int j=0;j<superBlockSparseTable[i].length;j++){
                superBlockSparseTable[i][j]=
                        L.get(superBlockSparseTable[i-1][j])<=L.get(superBlockSparseTable[i-1][j+(int)Math.pow(2,i-1)])?
            ...
            for (int j=1;j<blockSparseTable[i].length;j++){
                ...
                for (int k=0;k<blockSparseTable[i][j].length;k++){
                    blockSparseTable[i][j][k]=
                            L.get(blockSparseTable[i][j-1][k])<=L.get(blockSparseTable[i][j-1][k+(int)Math.pow(2,j-1)])?
                                    blockSparseTable[i][j-1][k]:blockSparseTable[i][j-1][k+(int)Math.pow(2,j-1)];
                }
            }
        }
    }
    private static void computeAllAnswers(){
        /**
        ...
        for (int i=0;i<=L.size()/blockSize;i++){
    ...
    private static int answerSuperblockQuery(int i, int j){
        /**
    ...
    private static int getSuperBlockMinimum(int left, int right){
        /**
    ...
    private static int getBlockMinimum(int left, int right){
        /**
    ...
    private static int getBlockType(int blockIndex){
        /**
    ...
    private static int getMinimum(int index1, int index2){
        /**
         ...
         */
        return L.get(index1)<=L.get(index2)?index1:index2;
    }
}
